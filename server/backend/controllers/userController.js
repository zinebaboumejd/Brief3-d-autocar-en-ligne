...
// fonction afficher voyage avec affichage propriater car avec id car
const AfficherVoyage = asyncHandler(async (req, res) => {
  // const voyage = await Voyage.find({}).populate({
  //   path: "idcar",
  //   select: "marque",
  // });
  const voyage = await Voyage.find().populate('idcar');
  res.json(voyage);
  })


// fonction afficher voyage par id
const AfficherVoyageById = asyncHandler(async (req, res) => {
  const voyage = await Voyage.findById(req.params.id);
  res.json(voyage);
});
// recherche voyage par destination ou origine ou date ou les trois

const RechercheVoyage = async (req, res) => {
